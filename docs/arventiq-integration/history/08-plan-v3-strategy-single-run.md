...
### 3.3 `ExecutionStrategy` — thin, delegates comparison out
...
```go
strategy := strategyRegistry.Resolve(problem.Type) // mirrors AdapterRegistry.Resolve(language)
```

Keeps dispatch logic (which `ProblemType` maps to which `ExecutionStrategy` implementation) out of `main.go`/`central_runner.go` business logic, in a registry that new problem models register into — same shape as the existing language adapter registry, so anyone already familiar with that pattern already understands this one.
