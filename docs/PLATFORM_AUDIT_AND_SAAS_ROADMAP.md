...
- ~~Stripe (or equivalent) subscription integration, tied to the Phase 1 tenant model and metering events.~~ **Scaffolded.** `assessment-api/src/config/plans.js` defines a `free`/`pro` plan map (placeholder limits); `College.mjs` grew `planId`/`stripeCustomerId`/`stripeSubscriptionId`/`subscriptionStatus`/`currentPeriodEnd`; `billing.service.js` + `billing.routes.js` add checkout/portal/webhook/status endpoints. `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`/`STRIPE_PRICE_ID_PRO` are unset in production — billing routes soft-fail with `503` rather than crashing the app (deliberately **not** the `JWT_SECRET`-style boot-time fail-fast, since billing isn't live yet).
- ~~Self-serve signup/onboarding flow~~ **Done.** `POST /api/auth/signup` (public, rate-limited via the existing `authLimiter`) creates a new `College` + its first `admin` user atomically via `college.service.js`, reusing `authService.register` — role/collegeId are always server-derived, never client-supplied. Frontend: `SignupPage.jsx` at `/signup`, linked from `LoginPage.jsx`. **Not done**: an admin console for plan/seat management beyond the basic `BillingPage.jsx` (view plan/usage, upgrade, manage-billing-portal link) — inviting additional users into an existing college, seat-level management UI, etc. is still open.
...
**Still needed before this can actually go live**: a real Stripe account + test/live keys, a real
pricing decision (the `free`/`pro` limits in `plans.js` are placeholders), and
`BILLING_SUCCESS_URL`/`BILLING_CANCEL_URL` pointed at the real frontend origin. The
checkout/webhook/portal flows are implemented but unverified end-to-end against real Stripe —
same "implemented but needs a live environment to verify" status as C6's deferred
docker-socket-proxy work.
